...
exports.getCategoryById = (categoryId, data = { _id: 1 }) => {
  const query = {
...
exports.getAllCategory = (data = { title: 1,}) => {
  return DbRepo.find(collections.FAQCATEGORY, { data })
}
...
exports.checkCategoryExistById = async (categoryId, data = { _id: 1 }) => {
  try {
    ...
    const category = await DbRepo.findOne(collections.FAQCATEGORY, { query, data })
